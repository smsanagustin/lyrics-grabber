...
const USER_AGENT =
    'Lyrics Grabber (GNOME Shell extension; https://github.com/sophiathesenpai/lyrics-grabber)';
...
function simplifyTitle(title) {
    return title
        .replace(/\s*[-–—]\s*[^-–—]*\b(version|remaster(ed)?|remix|live|edit|mix|mono|stereo|deluxe|acoustic)\b.*$/i, '')
        .replace(/\s*\([^)]*\b(version|remaster(ed)?|remix|live|edit|mix|mono|stereo|deluxe|acoustic|feat\.?|ft\.?)\b[^)]*\)\s*/gi, ' ')
        .replace(/\s+/g, ' ')
        .trim();
}
